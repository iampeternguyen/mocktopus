...
async function loadAllSwaggerFiles(directory) {
    ...
    const swaggerFiles = files.filter(file => file.endsWith('.yml')|| file.endsWith('.yaml') || file.endsWith('.json'));
...
function findPathAndMethodInAll(swaggerDocs, requestedPath, method) {
    ...
    for (const swagger of swaggerDocs) {
        ...
        const similarity = stringSimilarity.compareTwoStrings(requestedPath, swaggerPath);
        ...
        if (similarity > highestSimilarity) {
            ...
            bestMatch = { swagger, path: swaggerPath, method: lowerCaseMethod, operation: methods[lowerCaseMethod] };
        ...
        const response = await axios.post(
            chatGptAPIUrl,
            {
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are an assistant that generates JSON responses based on provided schemas."
                    },
                    {
                        role: "user",
                        content: prompt
...
async function handleRequest(requestedPath, method) {
    try {
        ...
        const matchedOperation = findPathAndMethodInAll(swaggerDocs, requestedPath, method);
        ...
        if (schema) {
            ...
            console.log('Generated AI Response:', JSON.stringify(aiResponse, null, 2));
            return aiResponse;
        } else {
